...
use Enmaca\Backoffice\FontManager\UI\Catalog\Content as FontManagerCatalogContent;

class TypographyCatalogController extends \App\Http\Controllers\Controller
{
    ...
    public function __invoke(Request $request, MasterLayout $layout): string
    {
        ...
        $layout->setTitle('Catálogo de Tipografías');
        ...
        try {
            $layout->content(
                UI::card('digitalProductCard')
                    ->body(
                        FontManagerCatalogContent::getMainContent()
                    )
            );
        ...
        $layout->viteAsset('resources/js/typography-catalog.js');
